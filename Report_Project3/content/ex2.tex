...
\begin{figure}[H]
    ...
    \node[block, minimum width=4cm, fill=yellow!30, right=0pt of b2] (ps) {PS (random $\geq$ 8 bytes)};
    ...
    % Nhãn tổng thể
    \node[above=0.5cm of ps] {EM = $k$ bytes (bằng kích thước khóa)};
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        io/.style={draw, rectangle, rounded corners=8pt, minimum width=2.8cm, minimum height=0.7cm, align=center},
...
\Function{RSA\_Encrypt}{pubKeyFile, plainFile, cipherFile}
    ...
    \For{$i \gets 0$ \textbf{to} len(plaintext) \textbf{step} maxBlockSize}
        ...
        \State cipherBlock $\gets$ publicKey.encrypt(block, PKCS1v15())
        \State cipherBlocks.append(cipherBlock)
    \EndFor
    
    \State \Call{WriteFile}{cipherFile, Concat(cipherBlocks)}
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\Function{RSA\_Decrypt}{privKeyFile, cipherFile, plainFile}
    ...
    \For{$i \gets 0$ \textbf{to} numBlocks $-$ 1}
        ...
        \State plainBlock $\gets$ privateKey.decrypt(block, PKCS1v15())
        \State plaintextBlocks.append(plainBlock)
    \EndFor
    
    \State \Call{WriteFile}{plainFile, Concat(plaintextBlocks)}
\EndFunction
\end{algorithmic}
\end{algorithm}
